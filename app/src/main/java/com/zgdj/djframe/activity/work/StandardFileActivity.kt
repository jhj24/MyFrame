...
import com.zgdj.djframe.adapter.StandardFileAdapter
...
class StandardFileActivity : BaseNormalActivity(), INotifyListener {


    private lateinit var listDialog: ListViewDialog
    private lateinit var loadingDialog: LoadingDialog //loading dialog
    private lateinit var footerView: View
    private lateinit var adapter: StandardFileAdapter
    private lateinit var list: MutableList<StandardFileBean.DataBean>
    ...
    private fun getListInfoTask(search: String = "") {
        ...
        val params = RequestParams(Constant.URL_WORK_STANDARD_LIST)
        params.addHeader("token", token)
        params.addBodyParameter("nodeId", key.toString())//用户id
        ...
        x.http().post(params, object : Callback.CommonCallback<String> {
            ...
            override fun onSuccess(result: String?) {
                ...
                val model = Gson().fromJson<StandardFileBean>(result, StandardFileBean::class.java)
                if (model.code != 1) {
                    if (model.code == -2) {
