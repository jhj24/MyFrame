...
public class AnimManager {
    ...
    private void setBottomToTopAnim(RecyclerView recyclerView, int position, float percent) {
        ...
        if (percent <= 0.5) {
            if (mLeftView != null) {
                Log.d(TAG, "mLeftView=" + ((1 - mAnimFactor) + percent * mAnimFactor) + "; position=" + position + "; percent=" + percent);
                mLeftView.setScaleX((1 - mAnimFactor) + percent * mAnimFactor);
                mLeftView.setScaleY((1 - mAnimFactor) + percent * mAnimFactor);
            }
            if (mCurView != null) {
                Log.d(TAG, "mCurView=" + (1 - percent * mAnimFactor) + "; position=" + position + "; percent=" + percent);
                mCurView.setScaleX(1 - percent * mAnimFactor);
                mCurView.setScaleY(1 - percent * mAnimFactor);
            }
            if (mRightView != null) {
                Log.d(TAG, "mRightView=" + ((1 - mAnimFactor) + percent * mAnimFactor) + "; position=" + position + "; percent=" + percent);
                mRightView.setScaleX((1 - mAnimFactor) + percent * mAnimFactor);
                mRightView.setScaleY((1 - mAnimFactor) + percent * mAnimFactor);
            }
        } else {
            if (mLeftView != null) {
                Log.d(TAG, "mLeftView=" + (1 - percent * mAnimFactor) + "; position=" + position + "; percent=" + percent);
                mLeftView.setScaleX(1 - percent * mAnimFactor);
                mLeftView.setScaleY(1 - percent * mAnimFactor);
            }
            if (mCurView != null) {
                Log.d(TAG, "mCurView=" + ((1 - mAnimFactor) + percent * mAnimFactor) + "; position=" + position + "; percent=" + percent);
                mCurView.setScaleX((1 - mAnimFactor) + percent * mAnimFactor);
                mCurView.setScaleY((1 - mAnimFactor) + percent * mAnimFactor);
            }
            if (mRightView != null) {
                Log.d(TAG, "mRightView=" + (1 - percent * mAnimFactor) + "; position=" + position + "; percent=" + percent);
