package com.zgdj.djframe.view.gallery;
...
public class GalleryItemDecoration extends RecyclerView.ItemDecoration {
    ...
    private void onSetVerticalParams(ViewGroup parent, View itemView, int position, int itemCount) {
        ...
        int itemNewHeight = parent.getHeight() - Utils.dp2px(context, 4 * mPageMargin + 2 * mLeftPageVisibleWidth);

        mItemComusemY = itemNewHeight + Utils.dp2px(context, 2 * mPageMargin);


        // 适配第0页和最后一页没有左页面和右页面，让他们保持左边距和右边距和其他项一样
        int topMargin = position == 0 ? Utils.dp2px(context, mLeftPageVisibleWidth + 2 * mPageMargin) : Utils.dp2px(context, mPageMargin);
        int bottomMargin = position == itemCount - 1 ? Utils.dp2px(context, mLeftPageVisibleWidth + 2 * mPageMargin) : Utils.dp2px(context, mPageMargin);
    ...
    private void onSetHoritiontalParams(ViewGroup parent, View itemView, int position, int itemCount) {
        int itemNewWidth = parent.getWidth() - Utils.dp2px(context, 4 * mPageMargin + 2 * mLeftPageVisibleWidth);
        int itemNewHeight = parent.getHeight();

        mItemComusemX = itemNewWidth + Utils.dp2px(context, 2 * mPageMargin);


        // 适配第0页和最后一页没有左页面和右页面，让他们保持左边距和右边距和其他项一样
        int leftMargin = position == 0 ? Utils.dp2px(context, mLeftPageVisibleWidth + 2 * mPageMargin) : Utils.dp2px(context, mPageMargin);
        int rightMargin = position == itemCount - 1 ? Utils.dp2px(context, mLeftPageVisibleWidth + 2 * mPageMargin) : Utils.dp2px(context, mPageMargin);
